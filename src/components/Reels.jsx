...
export default function Reels({ reels, reelRefs, showGreen, showRed, spinning }) {
  return reels.map((reel, i) => (
    <React.Fragment key={i}>
      <Reel key={i} ref={reelRefs[i]} reel={reel} showGreen={showGreen} showRed={showRed} reelIndex={i} spinning={spinning} />
      {i < 2 && (
...
const Reel = React.forwardRef(({ reel, spinning, showGreen, showRed, reelIndex }, ref) => {
      ...
      <div className="reel_inner">
        <div ref={ref} className={`reel_strip ${spinning ? "spinning" : ""}`}>
          {doubledReel.map((symbol, i) =>
            symbol === "Blank" ? (
              <div id={`reel_${reelIndex}_pos_${i}`} style={{ height: 250, width: "100%", backgroundColor: "transparent" }} key={i} />
            ) : (
